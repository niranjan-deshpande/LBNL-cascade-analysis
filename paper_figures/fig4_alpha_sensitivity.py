"""Figure 4: α-sensitivity — does the W=12 cluster-bound headline survive
varying the local/network split parameter α_local?

Three points (α ∈ {0.05, 0.15, 0.30}) with 95% CI bars at each. Horizontal
reference at zero; horizontal band at the +30/yr W=12 headline value.

Reads:  ABM/full_abm/output/alpha_sensitivity_summary.csv
Writes: paper_figures/fig4_alpha_sensitivity.{pdf,png}
"""
...
OUT_PDF = os.path.join(ROOT, "paper_figures/fig4_alpha_sensitivity.pdf")
OUT_PNG = os.path.join(ROOT, "paper_figures/fig4_alpha_sensitivity.png")
...
CORAL = "#CC785C"
BROWN = "#8C6F5A"
HEADLINE = 29.85   # W=12 default-α headline (+30/yr PJM)


def main():
    df = pd.read_csv(ALPHA_CSV).sort_values("alpha").reset_index(drop=True)
    ...
    ax.errorbar(a, y, yerr=yerr, fmt="none", ecolor=CORAL,
                elinewidth=1.4, capsize=5, capthick=1.2, zorder=3)
    ax.plot(a, y, linestyle="none",
            marker="o", markersize=9,
            markerfacecolor=CREAM, markeredgecolor=CORAL,
            markeredgewidth=1.7, zorder=4)

    # Headline label
    ax.text(0.355, HEADLINE + 0.6, f"+{HEADLINE:.0f}/yr (default α)",
            fontsize=8.8, color=BROWN, ha="right", va="bottom",
            style="italic")

    # Per-α value labels
    for ai, yi, yse in zip(a, y, se):
        ax.text(ai, yi + 1.96 * yse + 1.5,
                f"+{yi:.1f}", ha="center", va="bottom",
                fontsize=9, color=INK)

    ax.set_xticks([0.05, 0.15, 0.30])
    ax.set_xlim(0.005, 0.35)
    ax.set_xlabel(r"local share $\alpha_\text{local}$", fontsize=10.5, color=INK)
    ...
    legend_handles = [
        ...
        Line2D([0], [0], color=BROWN, lw=0.8, linestyle=(0, (3, 2)),
               label="default-α headline (+30/yr)"),
    ]
    ...
    ax.set_title("Robustness to α (local/network split)",
