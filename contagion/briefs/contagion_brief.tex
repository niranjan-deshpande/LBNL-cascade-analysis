...
\textbf{Scope: within-POI cascades as a lower bound.} This study identifies contagion \emph{within} a single POI, but the underlying cost-reallocation mechanism is primarily network-level, not POI-local. Structurally, only roughly 10--15\% of shared interconnection costs are truly POI-local (Attachment Facilities / TOIF); the remaining 85--90\% are network upgrades driven by power-flow contributions and are shared across a broader set of contributing projects, a large (if unquantified) fraction of which sit at \emph{different} POIs from the triggering withdrawal. A cross-POI design that tracked flow-based cost-sharing would in principle capture that larger channel, but the matching problem becomes substantially harder: the relevant peer set is defined by power-flow distribution factors rather than co-location, controls are not cleanly identified, and the treatment is diffuse. We therefore frame the within-POI matched DiD as a defensible \textbf{lower bound} on the true magnitude of cost-reallocation-induced cascading. The ABM that follows likewise models only the within-POI channel and should be read as a lower-bound structural analogue.
